...
function App() {
  ...
  function deleteCocktailCard(idToDelete) {
    for (let i = 0; i < cocktailList.length; i++) {
      if (cocktailList[i].id === idToDelete) {
        const delCocktail = [
          ...cocktailList.slice(0, i),
          ...cocktailList.slice(i + 1),
        ];
  ...
  return (
    <div className="App">
      ...
      <button id="button" className="buttons" onClick={handleClick}>
        Click here for cocktail inspo
      </button>
      <button id="clear" className="buttons" onClick={clearAll}>
        Clear all
      </button>
      <h2 className="h">My list of cocktails to try</h2>
      <CardList
        cocktailList={cocktailList}
        deleteCocktail={deleteCocktailCard}
      />
    </div>
  );
}
